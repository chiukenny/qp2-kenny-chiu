...
\end{itemize}
In the equal size case, the number of individuals in each cluster at each time point is $N=100$. In the unequal size case, for each simulation, the parameter of a multinomial distribution is sampled from a Dirichlet distribution with parameters $(1,\ldots,1)$. A multinomial distribution with this parameter is then used to randomly assign $N(I-1)=2376$ individuals across $I$ clusters. An additional individual is added to each cluster afterwards (in order to avoid empty clusters), making the total number of individuals across clusters equal to $NI=2400$. Each cluster keeps the same number of individuals across time points.
\\
...
\]
where $\sqrt{\Var(\hat{\theta})}$ is either taken as the standard error of $\hat{\theta}$ given in the outputs of the functions (which we assume is what \citeauthor{Hussey:2007} refer to as the ``standard variance'') or by a jackknife estimate. The jackknife estimate is computed by \todo. We reject the null hypothesis if $|W|>z(0.975)$ where $z(0.975)$ is the 97.5\% quantile of the standard normal distribution. The power is then estimated by the proportion of rejects out of the 1000 simulations. \todo t test?
