...
\]
For both GEE and GLMM, we specify the link function to be the identity. Therefore in all three models, the mean response for an individual in the control phase is given by $\mu + \beta_j$ and for an individual in the intervention phase is given by $\mu+\beta_j+\theta$. This allows the treatment effect $\theta$ to be interpreted similarly across the three approaches so that the powers are comparable. We note that \citeauthor{Hussey:2007} do not clarify what link function they use and so it is possible that they use the default logit link.
...
\]
where $\sqrt{\Var(\hat{\theta})}$ is taken as the standard error of $\hat{\theta}$ given in the outputs of the functions (which we assume is what \citeauthor{Hussey:2007} refer to as the ``standard variance''). For GEE, we use the output robust (Huber-White) standard error. Note that \citeauthor{Hussey:2007} also compute the power using a jackknife estimate of the variance. Due to time constraints on this report, we do not replicate this component of the study and instead provide a discussion in Secion~\ref{sec:jackknife}. We reject the null hypothesis if $|W|>z(0.975)$ where $z(0.975)$ is the 97.5\% quantile of the standard normal distribution. The power is then estimated by the proportion of rejects out of the 1000 simulations. In the unequal cluster size case, we found rare instances where the LMM failed to be fit. In this case, the LMM power is estimated by the proportion of rejects out of the number of non-failing simulations.
...
Our results for when time effects are included in the model are shown in Table~\ref{tab:simulation}. We note that our reported powers are generally consistent or otherwise greater than the powers reported by \citeauthor{Hussey:2007}. In particular, our powers for GLMM with an identity link function are generally much greater when $\text{RR}<1$ in both the equal and unequal cluster size case. The powers for GLMM with the default logit link function are smaller and arguably more consistent with that of \citeauthor{Hussey:2007}'.
...
\midrule
1.0 &&  0.050 & 0.089 & 0.081 && 0.066 & 0.052 && 0.062 & 0.11 &  && 0.058 & 0.053 \\
0.7 && 0.700 & 0.736 & 0.723 && 0.805 & 0.711 && 0.345 & 0.70 & && 0.779 & 0.688 \\
0.6 && 0.920 & 0.928 & 0.920 && 0.963 & 0.929 && 0.536 & 0.95 & && 0.951 & 0.913 \\
0.5 && 0.981 & 0.983 & 0.982 && 0.994 & 0.985 && 0.719$^{**}$ & 0.99 & && 0.997 & 0.985 \\
\bottomrule
\end{tabular}
\caption{The powers for testing the treatment effect in LMM, GEE, and GLMM over 1000 simulations. The power is computed using the standard error provided in the function output (robust SE in the case of GEE). Powers for GEE and GLMM with identity (id) and logit link functions are reported. $^{**}$LMM failed to fit the model on two simulated datasets and so this power was approximated over 998 simulations.}
...
Due to time constraints on this report, we do not replicate the power simulation study with jackknife estimates of the variance. \citeauthor{Hussey:2007} also are unclear about how they compute the jackknife estimate. We propose an jackknife estimator of the variance that we believe would be reasonable and may be the one that they used. In the context of our simulation, we assume that cluster sizes may vary, but the size remains the same over time for each cluster.
\\

\todo We first give a jackknife estimate of the treatment effect. Let $\bfy$ denote the data collected from the SW-CRT. Let $\bfy_{-i}$ denote the individual data for all clusters but cluster $i$ across all time points. Let $\hat{\theta}(\argdot)$ denote the treatment effect estimator given data $\argdot$. Let $M=\sum_{i=1}^IN_i$.

\[
\hat{\theta}_{JK} = I\hat{\theta}(\bfy) - \sum_{i=1}^I\left(\frac{M-N_i}{M}\right)\hat{\theta}(\bfy_{-i})
\]
...
\]
average pseudo-value
\[
\bar{\hat{\theta}} = \frac{1}{M}\sum_{i=1}^I N_i\hat{\theta}_i = I\hat{\theta}(\bfy) - \sum_{i=1}^I\frac{(M-N_i)}{M}\hat{\theta}(\bfy_{-i})
\]

variance estimate?
\[
\hat{\Var}(\hat{\theta}_{JK}) = \frac{1}{M^2}\sum_{i=1}^IN_i^2(\hat{\theta}_i-\bar{\hat{\theta}})^2
\]
